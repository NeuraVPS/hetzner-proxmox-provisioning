...
# regenerated by sync-dnat.py from /etc/pve/qemu-server/*.conf). NOTE: the
# pin file lives OUTSIDE /etc/dnsmasq.d/ on purpose — SIGHUP re-reads files
# referenced by dhcp-hostsfile=, but does NOT re-read /etc/dnsmasq.d/*.conf,
# so a pin file under /etc/dnsmasq.d/ would only take effect on full restart.
# RA flags default in this mode: M=1 (DHCPv6 for addresses), A=0 (no SLAAC),
# O=1 (DHCPv6 for DNS). Windows receives ONLY the pinned <prefix>::<vmid_hex>
# via DHCPv6 SOLICIT — server-authoritative, self-healing on every boot.
enable-ra
...
# Pin file (referenced by dhcp-hostsfile= above). Starts empty; populated by
# sync-dnat.py on every VM lifecycle event (post-start/post-stop/node-boot).
: > /etc/dnsmasq-vm-pins.hosts

# Remove legacy pin file from when pins lived inside /etc/dnsmasq.d/ (where
# SIGHUP didn't re-read them).
