...
' line, '#' comments and blanks ignored. Written once per VM by a fleet sweep
' that decides from the AppData side (origin.txt + EA counts + terminal logs)
' rather than guessing at launch.
'
...
' any launch-time heuristic keeps hiding the customer's real data. The
' decision has to be made with the AppData side in view, and only once.
'
...
' ResolveMT5Target may have redirected above. /update* runs keep the original
' command line untouched, as before this opt-out existed.
