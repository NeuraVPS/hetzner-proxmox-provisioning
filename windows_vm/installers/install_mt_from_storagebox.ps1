...
try {
    ...
    if ($StartMinimized) {
        # MT5 ignores shortcut / STARTUPINFO minimise hints and restores its own saved window
        # placement, so "start minimised on reboot" cannot be done from the launch shortcut.
        # Drop a per-logon helper that runs IN the interactive session (where the terminal's
        # MainWindowHandle is visible, unlike SYSTEM/session 0) and minimises each terminal
        # window once as it appears. Uses only user32 ShowWindowAsync via Add-Type -- no
        # compiled binary, so it is not subject to the Defender content-signature deletion that
        # blocks a standalone injector exe. Runs every logon => minimised on every reboot.
        ...
        $minScript = @'
# NeuraVPS DFY image: keep MetaTrader terminals minimised after logon.
# Generated by install_mt_from_storagebox.ps1 (-StartMinimized). Runs hidden from an
# all-users Startup shortcut, in the interactive logon session.
$ErrorActionPreference = 'SilentlyContinue'
Add-Type -TypeDefinition 'using System;using System.Runtime.InteropServices;public class NvWin{[DllImport("user32.dll")]public static extern bool ShowWindowAsync(IntPtr h,int c);}'
$SW_FORCEMINIMIZE = 11
$seen = @{}
$deadline = (Get-Date).AddSeconds(60)
while ((Get-Date) -lt $deadline) {
    foreach ($p in (Get-Process '__PROCNAME__' -ErrorAction SilentlyContinue)) {
        $h = $p.MainWindowHandle
        if ($h -ne [IntPtr]::Zero -and -not $seen.ContainsKey([int64]$h)) {
            [void][NvWin]::ShowWindowAsync($h, $SW_FORCEMINIMIZE)   # minimise once; never fight the user after
            $seen[[int64]$h] = $true
        }
    }
...
'@
        $minScript = $minScript.Replace('__PROCNAME__', $procName)
        $minPath = Join-Path -Path $neuraDir -ChildPath 'mt_minimize.ps1'
        $utf8Bom = New-Object System.Text.UTF8Encoding($true)   # BOM so Windows PowerShell 5.1 reads it as UTF-8
        [System.IO.File]::WriteAllText($minPath, $minScript, $utf8Bom)

        $winPs = Join-Path $env:SystemRoot 'System32\WindowsPowerShell\v1.0\powershell.exe'
        $minLnk = Join-Path -Path $AllUsersStartup -ChildPath 'NeuraVPS MT Minimize.lnk'
        $minArgs = '-NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File "{0}"' -f $minPath
        New-MtShellShortcut -ShortcutPath $minLnk -TargetPath $winPs -WorkingDirectory $neuraDir -Arguments $minArgs -WindowStyle 7
    }
...
if ($StartMinimized)          { $summary += ' Start-minimised helper applied.' }
