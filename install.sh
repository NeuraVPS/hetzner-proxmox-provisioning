...
# Aislamiento de CAPA 3 (vmbr0<->vmbr0). nvx-aisla corta VM<->VM por capa 2;
# esto cierra el atajo L3 (origen ajeno enrutado a la vecina por vmbr0).
# Nace en modo drop (canario de flota validado 18/09/2026). MODE por drop-in.
cat > /mnt/usr/local/sbin/nvx-aisla-l3.sh <<'NVXEOF'
#!/usr/bin/env bash
...
NVXEOF
chmod 755 /mnt/usr/local/sbin/nvx-aisla-l3.sh

cat > /mnt/etc/systemd/system/nvx-aisla-l3.service <<'NVXEOF'
[Unit]
...
chroot /mnt systemctl enable nvx-aisla-l3.service >/dev/null 2>&1 || true
